...
function http_deep(ip, port, use_tls)
    ...
    hdr = try
        read(`$CURL -sL --max-time $DEEP_TOUT --compressed -I $uri`, String)
    catch
        ""
    end
    body = try
        read(`$CURL -sL --max-time $DEEP_TOUT --compressed $uri`, String)
    catch
        ""
    end
    ...
    rob = try
        read(`$CURL -sL --max-time 2 $uri/robots.txt`, String)
    catch
        ""
    end
...
function port_scan(ips::Vector{String}, ports, to)
    ...
    for ip in ips
        ...
        @threads for i in 1:length(ports)
            ...
            if open && dr !== nothing
                dr.extras["security"] = judge_security(dr)
            end
            res[i] = (p, open, dr)
        end
        results[ip] = res
